...
const normalizedNumbers = populations.map((el)=>+el.innerHTML.split(',').join(''));
const nfObject = new Intl.NumberFormat('en-US');
const totalPopulation = normalizedNumbers.reduce((prev, curr)=>prev + curr, 0);
const averagePopulation = +(totalPopulation / normalizedNumbers.length).toFixed(0);
